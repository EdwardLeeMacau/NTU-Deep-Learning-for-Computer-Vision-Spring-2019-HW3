...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            nn.ConvTranspose2d(100, opt.img_size * 8, 4, 1, 0, bias=False),
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            nn.Conv2d(opt.channels, opt.img_size, 4, 2, 1, bias=False),
            nn.LeakyReLU(0.2, inplace=True),
            
            nn.Conv2d(opt.img_size, opt.img_size * 2, 4, 2, 1, bias=False),
            nn.BatchNorm2d(opt.img_size * 2),
            nn.LeakyReLU(0.2, inplace=True),
            ...
            nn.Conv2d(opt.img_size * 4, opt.img_size * 8, 4, 2, 1, bias=False),
            nn.BatchNorm2d(opt.img_size * 8),
            nn.LeakyReLU(0.2, inplace=True),
...
def train(epoch):
    ...
    for i, (imgs, _) in enumerate(dataloader, 1):
        ...
        if batches_done % opt.save_interval == 0:
            ...
            print("Model saved to: {}, iteration: {}".format("./models/gan/{}".format(opt.tag), number))
...
def main():
    ...
    for epoch in range(1, opt.n_epochs):
        ...
        plt.plot(g_loss_iteration, label='G loss', color='r', linewidth=0.25)
        plt.plot(d_loss_iteration, label='D loss', color='b', linewidth=0.25)
        plt.legend(loc=0)
        plt.ylabel("Loss")
